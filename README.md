# DWSTUDIO-NanugiStudio

**DWSTUDIO** 테마 v1.0 – Nanugi Studio 스토어 전용 (Tinker 기반).  
...
- **에디터**: [테마 에디터](https://admin.shopify.com/store/nanugi/themes/158825414873/editor)  
...
`main` 브랜치에 푸시하면 자동으로 테마(158825414873)에 배포됩니다.
...
2. **Environment에 Secrets 추가**  
   ...
   | `SHOPIFY_THEME_ID` | 배포할 테마 ID. 예: `158825414873` |
...
- 앱 이름 입력 (예: `Nanugi Studio Theme Deploy`) 후 **Create** 클릭
...
- **Custom distribution** 선택 → **Select**  
  - 이 앱은 본인 스토어(Nanugi Studio)용이므로 **Custom**이 맞습니다.  
  - Public은 앱스토어 공개용이라 선택하지 않습니다.  
- Custom 선택 후 스토어 도메인(`nanugi.myshopify.com`) 입력하고 **Generate link**로 설치 링크를 만들 수 있습니다(선택).

### 5. 스토어에 앱 설치

- **Install** 버튼 클릭  
- **Select store**에서 **Nanugi Studio** 스토어 선택 (또는 사용할 스토어)  
...
- GitHub 저장소 → **Settings** → **Environments** → **shopify-nanugi**  
