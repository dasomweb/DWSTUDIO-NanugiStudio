# Nanugi Studio 테마 배포 가이드 (Wiki)

이 문서는 **GitHub Actions**와 **Shopify Dev Dashboard 앱**을 사용해 Nanugi Studio 스토어 테마를 배포하는 방식을 설명합니다.
...
[GitHub Actions] ① 저장소 체크아웃
         ...
         ↓
[Shopify] 지정한 테마(158823317721)에 파일 반영
```

- **토큰:** Theme Access/레거시 앱의 장기 토큰을 쓰지 않고, **매 배포마다** Client credentials로 새 액세스 토큰을 받아 사용합니다.
- **대상:** Environment `shopify-nanugi`의 Secrets에 저장한 스토어·테마 ID로만 배포됩니다.
...
- 앱 이름 입력 (예: `Theme Push`, `Nanugi Studio Theme Deploy`) 후 **Create** 클릭
...
2. **Custom distribution** 선택 → **Select**
   - 이 앱은 Nanugi Studio 한 스토어(또는 정해진 스토어)용이므로 **Custom**이 맞습니다.
   - **Public**은 앱스토어 공개용이므로 선택하지 않습니다.
3. (선택) Custom 선택 후 스토어 도메인(`nanugi.myshopify.com`) 입력하고 **Generate link**로 설치 링크 생성 가능

### 4.5 스토어에 앱 설치

- **Install** 버튼 클릭
- **Select store**에서 **Nanugi Studio** 스토어 선택
...
배포는 **Environment** `shopify-nanugi`에 연결되어 있으며, 이 Environment의 **Secrets**만 사용합니다.

### 5.1 Environment 생성 (없는 경우)

- GitHub 저장소 → **Settings** → **Environments**
- **New environment** 클릭 후 이름에 **shopify-nanugi** 입력하여 생성

### 5.2 Environment secrets 추가

- **shopify-nanugi** Environment 선택 → **Environment secrets**에서 **Add secret**로 아래 4개 추가:
...
| `SHOPIFY_THEME_ID` | `158823317721` | 배포할 테마 ID (에디터 URL에 포함) |
...
배포가 성공하면 지정한 테마(158823317721)에 변경 사항이 반영됩니다.  
테마 에디터: [Nanugi Studio 테마 에디터](https://admin.shopify.com/store/nanugi/themes/158823317721/editor)
...
cd H:\GitHub\DWSTUDIO-NanugiStudio
shopify theme dev
```
...
| 다른 스토어/테마로 배포하고 싶음 | 새 Environment를 만들거나, 기존 Environment의 Secrets만 해당 스토어·테마·앱 값으로 교체. (워크플로는 동일하게 `shopify-nanugi` 사용 가능) |
...
| 항목 | 내용 |
|------|------|
| **테마 표시 이름** | `config/settings_schema.json` → `theme_name`: `"DWSTUDIO - NanugiStudio"` (Theme library 등에서 표시) |
